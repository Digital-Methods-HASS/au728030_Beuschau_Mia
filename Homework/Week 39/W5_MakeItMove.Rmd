...
```{r we look for the higest gdp in 1952}
gapminder%>%
...
```{r plot with color on continent}
options(scipen = 999)
ggplot(subset(gapminder, year == 2007), aes(gdpPercap, lifeExp, size = pop)) +
  geom_point() +
...
```{r we look for the higest gdp in 2007}
gapminder%>%
  filter(!is.na(gdpPercap))%>%
  filter(year == 2007)%>%
  arrange(desc(gdpPercap))
...
```{r indlæse gg animate}
library(gganimate)
```


```{r following a contry}
anim2 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop)) +
  geom_point() +
  scale_x_log10() +
  transition_time(year) +
...
how do you make a illustration of the development in Denmark?

løsning: 

```{r jeg finder data for danmark}
library(dplyr)
DK <- gapminder%>%
...
```{r plotting the data for denmark}
options(scipen=999)
